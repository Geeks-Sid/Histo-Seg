#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Fri Mar  8 20:03:35 2019

@author: siddhesh
"""

import numpy as np
from torch.utils.data.dataset import Dataset
from skimage.io import imread
from skimage.morphology import binary_closing, disk, binary_dilation, binary_erosion
from skimage.filters import threshold_otsu, median
from scipy.ndimage import binary_fill_holes
import PIL
from PIL import Image
import warnings

PIL.Image.MAX_IMAGE_PIXELS = 933120000
warnings.filterwarnings("ignore")


def tissue_mask_generation(img_rgb, rgb_min=50):
    """
    This function is used to generate tissue masks
    works for patches too i guess

    input :
        image : type : numpy.array : BGR format usually ex
        i
    """
    img_rgb = np.array(img_rgb)
    background_r = img_rgb[:, :, 0] > threshold_otsu(img_rgb[:, :, 0])
    background_g = img_rgb[:, :, 1] > threshold_otsu(img_rgb[:, :, 1])
    background_b = img_rgb[:, :, 2] > threshold_otsu(img_rgb[:, :, 2])
    tissue_rgb = np.logical_not(background_r & background_g & background_b)
    del background_b, background_g, background_r

    min_r = img_rgb[:, :, 0] > rgb_min
    min_g = img_rgb[:, :, 1] > rgb_min
    min_b = img_rgb[:, :, 2] > rgb_min
    tissue_mask = tissue_rgb & min_r & min_g & min_b
    del min_r, min_g, min_b

    close_kernel = np.ones((7, 7), dtype=np.uint8)
    image_close = binary_closing(np.array(tissue_mask), close_kernel)
    tissue_mask = binary_fill_holes(image_close)
    del image_close, close_kernel

    # Apply median filter
    tissue_mask = median(tissue_mask, disk(7))
    tissue_mask = np.array(tissue_mask).astype(np.uint8)
    tissue_mask = tissue_mask > 0

    return tissue_mask


class InferTumorSegDatasetJpg(Dataset):
    def __init__(self, wsi_path, patch_size, stride_size):
        self._wsi_path = wsi_path
        self._patch_size = int(patch_size)
        self._stride_size = int(stride_size)
        self._image = imread(self._wsi_path)
        self._points = []
        self._basic_preprocessing()

    def _basic_preprocessing(self):
        print("Initiating preprocessing of the Slide")
        height, width, _ = self._image.shape
        print("Image dimensions : ", (height, width))
        print("Generating Mask, this might take a while")
        mask = tissue_mask_generation(self._image)
        print("Total Area of mask : ", mask.sum())
        print("Approxiamate Tissue to Image ratio :", (mask.sum() / (height * width)))
        for i in range(0, width - self._patch_size, self._stride_size):
            for j in range(0, height - self._patch_size, self._stride_size):
                self._points.append([j, i])
        print("Number of points generated :", len(self._points))
        for i in range(len(self._points)):
            point = self._points[i]
            if not np.any(
                mask[
                    point[0] : point[0] + self._patch_size,
                    point[1] : point[1] + self._patch_size,
                ]
            ):
                self._points[i] = [-1, -1]
        self._points = np.delete(
            self._points, np.argwhere(self._points == np.array([-1, -1])), 0
        )
        print("Number of points after reduction :", len(self._points))
        del mask

    def __len__(self):
        return len(self._points)

    def __getitem__(self, idx):
        x_loc, y_loc = self._points[idx]
        patch = self._image[
            x_loc : x_loc + self._patch_size, y_loc : y_loc + self._patch_size
        ]
        patch = np.array(patch / 255, np.float32)
        patch = patch.transpose([2, 0, 1])
        return patch, (x_loc, y_loc)
